...
import {BuildRow, EnvVariables} from './types';
...
app.get('/', (c) => c.redirect('https://github.com/hampuskraft/discord-build-tracker', 302));

app.get('/stats', async (c) => {
  return c.json({
    total: await c.env.DB.prepare('SELECT COUNT(*) AS count FROM builds').first('count'),
    stable: await c.env.DB.prepare('SELECT COUNT(*) AS count FROM builds WHERE channel = ?')
      .bind(ReleaseChannel.Stable)
      .first('count'),
    ptb: await c.env.DB.prepare('SELECT COUNT(*) AS count FROM builds WHERE channel = ?')
      .bind(ReleaseChannel.Ptb)
      .first('count'),
    canary: await c.env.DB.prepare('SELECT COUNT(*) AS count FROM builds WHERE channel = ?')
      .bind(ReleaseChannel.Canary)
      .first('count'),
    rollback: await c.env.DB.prepare('SELECT COUNT(*) AS count FROM builds WHERE rollback = 1').first('count'),
  });
});

app.get('/latest/all', async (c) => {
  const stmt = c.env.DB.prepare('SELECT * FROM builds ORDER BY timestamp DESC LIMIT 1');
...
app.get('/latest/:type', async (c) => {
  const releaseChannel = z.nativeEnum(ReleaseChannel).parse(c.req.param('type'));
  const stmt = c.env.DB.prepare('SELECT * FROM builds WHERE channel = ? ORDER BY timestamp DESC LIMIT 1');
...
app.get('/search', async (c) => {
  ...
  const stmt = c.env.DB.prepare(
    [
      'SELECT * FROM builds',
      params.type || params.before || params.after || params.rollback != null ? 'WHERE 1 = 1' : '',
  ...
  const args = [params.type, params.before, params.after, params.rollback, params.limit].filter((arg) => arg != null);
